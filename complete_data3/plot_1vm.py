...
files = [ f for f in onlyfiles if "1vm_" in f]
print(files)
...
for f in files:
    ...
    for i in range(len(x)):
        ax1.scatter(x[i],hrs[i],s= ((1)%2)*6+5 ,label= sched[i] ,color=colrs[i],marker=maks[i])
        ax2.plot(x[i],cpus[i],color=colrs[i],lw=((i+1)%2)+3,label= sched[i],ls=lss[i] )
        # tmp=[]
    ...
    if show_frames:
        ...
        for j in range(len(frame_xs[i])):
            ...
            ax1.axvline(x=40,color='k', linestyle='-')
            ax1.axvline(x=80,color='k', linestyle='-')
            ax2.axvline(x=40,color='k', linestyle='-')
